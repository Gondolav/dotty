object Example {
  ...
  val url: Option[URL] = "https://www.epfl.ch/schools/ic/" match {
    // here if an optional is not present, its value is null
    case regexURL(protocol, _, hostname, _, domain, path) => Some(URL(Option(protocol), hostname, domain, Option(path))) // since the Regex extractor produces varargs, the user could use the wrong group without any safety check
    case _ => None
  }
  ...
  val compilationTime: Option[Int] = "Total time: 228 s, completed May 9, 2019 7:33:37 PM" match {
    case regexCompilationResult(_, time, x: _*) => Some(time.toInt) // if we call toInt on the wrong group, we get an exception at run-time
    case _ => None
  }
}